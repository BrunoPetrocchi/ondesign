...
class Artigos {
    ...
    public function  CadArtigos($titulo, $autor, $data, $artigo){
        $sql = "INSERT INTO posgraduacao.tblartigos values (NULL, '$titulo', '$autor', '$data', '$artigo')";
        return MysqlManager::ExecutaPersistenciaMysql($sql, $this->db);
    }
    
    public function  ExibirArtigos(){
        $sql="SELECT IDARTIGO, TITULO, AUTOR, DATA, ARTIGO FROM posgraduacao.tblartigos WHERE APAGADO = 1";
        return MysqlManager::ExecutaConsultaMysql($sql, $this->db);
    }
    
    public function ConsultarArtigos($id){
        $sql="SELECT TITULO, AUTOR, DATA, ARTIGO FROM posgraduacao.tblartigos  WHERE IDARTIGO = '$id' AND APAGADO = 1";
        return MysqlManager::ExecutaConsultaMysql($sql, $this->db);
    }
